...
abstract class HtmlForm[A, Html: Monoid]{
  def render(key: String, values: Input, errors: ErrorTree, messages: UniformMessages[Html], tell: Html = Monoid[Html].empty): Html
}

abstract class InferForm[Html: Monoid] {
  ...
  trait HtmlField[A]{
    ...
    implicit def simpleForm[A](implicit field: HtmlField[A]): HtmlForm[A, Html] = new HtmlForm[A, Html] {
