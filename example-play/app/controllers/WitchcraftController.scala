...
import ltbs.uniform._, interpreters.playframework._, examples.witchtax._
...
@Singleton
class WitchcraftController @Inject()(
  ...
  def witchAction(targetId: String) = Action.async { implicit request: Request[AnyContent] ⇒
    val playProgram = witchJourney(
